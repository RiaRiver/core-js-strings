...
function removeFirstOccurrences(str, value) {
  ...
  const valueStartInd = str.indexOf(value);

  return valueStartInd < 0
    ? str
    : str.slice(0, valueStartInd) + str.slice(valueStartInd + value.length);
}
...
function removeLastOccurrences(str, value) {
  const valueStartInd = str.lastIndexOf(value);
...
function formatTime(minutes, seconds) {
  const getFormattedPart = (part) => String(part).padStart(2, '0');

  return `${getFormattedPart(minutes)}:${getFormattedPart(seconds)}`;
}
...
function countVowels(str) {
  ...
  //   .reduce(
  //     (vowelsCount, char) =>
  //       vowels.includes(char.toLocaleLowerCase())
  //         ? vowelsCount + 1
  //         : vowelsCount,
  //     0
  //   );
...
function isPalindrome(str) {
  const normalizedStr = str.toLocaleLowerCase().replace(/\W/g, '');

  // return normalizedStr === normalizedStr.split('').reverse().join('');

  for (let i = 0; i < normalizedStr.length / 2; i += 1) {
    const forwardChar = normalizedStr.at(i);
    const backwardChar = normalizedStr.at(-(i + 1));
...
function encodeToRot13(str) {
  return str.replace(/(\w)/g, (char) =>
    String.fromCharCode(
      char.charCodeAt(0) + (char.toUpperCase() < 'N' ? 13 : -13)
    )
  );
}
